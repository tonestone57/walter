...
private:
	...
	mutable	Element* volatile fBest __attribute__((aligned(8)));
	...
	unsigned int priority)
{
	...
	{
		Element* best = (Element*)atomic_pointer_get<Element>(&fBest);
		if (best != NULL) {
...
RUN_QUEUE_CLASS_NAME::Remove(Element* element)
{
	...
	// freed while we hold the lock (object-cache reclaim requires the lock).
	{
		Element* best = (Element*)atomic_pointer_get<Element>(&fBest);
		if (best == element) {
			atomic_pointer_test_and_set<Element>(&fBest, (Element*)NULL, element);
		} else if (best != NULL) {
			...
			if (fHeads[bestPrio] == NULL)
				atomic_pointer_test_and_set<Element>(&fBest, (Element*)NULL, best);
		}
	}
}
...
{
	Element* bestCandidate = (Element*)atomic_pointer_get<Element>(&fBest);
	...
	// causing PeekBest to return a stale/dangling entry.
	{
		...
		atomic_pointer_test_and_set<Element>(&fBest, (Element*)NULL, bestCandidate);
	}
	...
	if (globalBest != NULL) {
		...
		while (best == NULL || sCompare(globalBest, best)) {
			Element* was = (Element*)atomic_pointer_test_and_set<Element>(&fBest,
				globalBest, best);
			if (was == best)
				break;
			best = was;
