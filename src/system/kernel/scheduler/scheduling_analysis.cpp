	...
	void* Allocate(size_t size) {
		...
		for (int32 i = 0; i < 1000; i++) {
#if B_HAIKU_64_BIT
			int64 current = (int64)atomic_get64(fNextAllocation);
			int64 newAlloc = current + (int64)size;
			int64 hashTableAddr = (int64)(uintptr_t)fHashTable;
			if (newAlloc > hashTableAddr)
				return NULL;
			if ((int64)atomic_test_and_set64(fNextAllocation, (int64)((uint64)newAlloc), (int64)current) == current) {
				atomic_add64(fRemainingBytes, (int64)- (int64)size);
				return (void*)(uintptr_t)current;
			}
#else
			int32 current32 =
				atomic_get(*reinterpret_cast<const int32 volatile*>(&fNextAllocation));
			...
			if (size > (size_t)B_INT32_MAX || newAlloc32 > hashTableAddr32)
				return NULL;
			if (atomic_test_and_set(*const_cast<int32 volatile*>(&fNextAllocation), (int32)(newAlloc32), (int32)(current32)) == current32) {
				atomic_add(*const_cast<int32 volatile*>(&fRemainingBytes), (int32)(-(int32)size));
